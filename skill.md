...
- Monthly chat volume? (from volumes / Gong)
- Current handoff to human agents? (from Gong / Granola)
- APIs or integrations needed? (from Gong / Granola)
- Segmentation rules? (from Gong / Salesforce)
- Chat-specific use cases? (from use cases discussion)

### Email Scoping Synthesis
- Email platform/system? (CRM / ticketing / Gong)
- Which email address(es) do customers contact? (from Gong / Granola)
- Webform or contact form? (from Gong / website context)
- How is email currently routed? (from Gong / tech stack)
- AI agent email address? (from scoping discussions)
- Gradual rollout needed? (from Gong / risk discussions)
- Email handoff to ticketing? (from Gong / Granola)

### Voice Scoping Synthesis
- Telephony provider? (Genesys, Avaya, Five9, Twilio, Vonage, etc.)
- CCaaS platform? (from tech stack / Gong)
- SIP integration type? (from Gong / technical discussions)
- Current IVR? (from Gong / Granola)
- Inbound or outbound or both? (from use cases)
- Call volume? (from volumes data)
- Agent count? (from Gong / Granola)
- Missed call rate? (from pain points / Gong)
- Primary voice use case? (from use cases)
- DTMF / dial pad needed? (from Gong / technical)
- SMS capabilities? (from Gong / requirements)
- Handoff to human agents? (from Gong / current process)
- Routing requirements? (from Gong / IVR discussion)
...
- **Bullet lists** (Gong highlights): use `-` on separate lines with a blank line before the list
- **Meeting notes**: bold title + date on one line, summary on next line(s), blank line between meetings
- **Project Scope**: Phase 1 / Phase 2 / Phase 3 each on a new line using `<br>`
- **Key client stakeholders**: one contact per line using `<br>`

Construct the full document content as Markdown with ALL placeholders replaced by real data.
...
| **KB Readiness** | [assessment from discussions or TBD] |
| **Project Scope** | Phase 1: [primary use case]<br>Phase 2: [secondary]<br>Phase 3: [tertiary] |
...
| Field | SC Input |
|---|---|
| **Current Chat Platform** | [e.g. Intercom, Zendesk Chat, Salesforce Chat] |
| **Monthly Chat Volume** | [from volumes data / Gong] |
| **Current Handoff Setup** | [describe current process from Gong/Granola] |
| **APIs / Integrations Required** | [from Gong/Granola API discussions] |
| **Segmentation Requirements** | [from discussions or TBD] |
| **Chat-Specific Use Cases** | [from use cases discussions] |
...
[IF email IS IN SCOPE: fill all table cells from data]

### Email Architecture

| Field | SC Input |
|---|---|
| **Tech Stack** | [same as chat or different — from tech stack] |
| **Email landscape** | [list email addresses from Gong/Granola] |
| **Webform** | [yes/no + description from Gong/Granola] |
| **Custom / Filter Incoming Emails** | [how they triage/filter incoming email] |
| **AI Agent / Human support** | [from scoping discussions or TBD] |
| **Launch plan** | [from rollout discussions or TBD] |

### Email Configuration

| Field | SC Input |
|---|---|
| **Knowledge Base** | [from KB discussions or TBD] |
| **Use cases** | [email-specific use cases from Gong/Granola] |
...
| Field | SC Input |
|---|---|
| **Email / Ticketing** | [how email tickets are created/routed] |
...
[IF voice IS IN SCOPE: fill all table cells from data]

### Voice Architecture

| Field | SC Input |
|---|---|
| **Telephony Provider** | [e.g. Genesys Cloud, Avaya, Five9, Twilio, Vonage] |
| **CCaaS / Agent System** | [full CCaaS platform name from tech stack / Gong] |
| **SIP Integration Type** | [from technical discussions or TBD] |
| **Current IVR** | [describe current IVR menu and flow] |
| **Inbound vs Outbound** | [inbound / outbound / both] |
...
| Field | SC Input |
|---|---|
| **Primary Voice Use Case** | [main reason customers call] |
| **Call Categorization / Triage** | [how calls are currently triaged] |
| **Secondary Voice Use Cases** | [additional voice use cases] |
...
| **DTMF / Dial Pad Input** | [yes/no + details] |
| **SMS Capabilities** | [yes/no + details] |
| **Cross-Channel Interoperability** | [cross-channel needs or TBD] |

### Voice Handoffs

| Field | SC Input |
|---|---|
| **Handoff to Human Agents** | [how voice calls hand off to humans] |
...
- **No local files** — do not save anything locally. Google Doc is the only output.
- **No Notion page** — Google Doc only.
- **ALL scoping sections always included** — General, Chat, Email, and Voice every time.
